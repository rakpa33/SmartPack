...
export function ColumnLayoutProvider({
  ...
  const toggleColumn = (columnId: ColumnId) => {
    setColumnVisibility(prev => {
      ...
      // Apply business rules after toggle
  ...
  const enforceVisibilityRules = (visibility: ColumnVisibility, device: DeviceType): ColumnVisibility => {
    ...
    if (visibleCount === 0) {
      return { ...visibility, packingChecklist: true }; // Default to packing checklist
    }

    // Rule 2: Mobile portrait = max 1 column
    if (device === 'mobile-portrait' && visibleCount > 1) {
      // Prioritize packing checklist > trip details > suggestions
    ...
    if (device === 'mobile-landscape' && visibleCount > 2) {
      // If all 3 are visible, hide suggestions (lowest priority)
  ...
  const MIN_COLUMN_WIDTH = 275;
  const DRAG_HANDLE_WIDTH = 8; // Approximate width of drag handles

  const getShrinkingPriority = (): ColumnId[] => {
    // Desktop horizontal shrinking priority: Suggestions → Trip Details → Packing Checklist
  ...
  const getResponsiveWidths = (): ColumnWidths => {
    ...
    // If all columns can fit at minimum width, distribute extra space
    ...
    if (usableWidth >= totalMinWidth) {
      ...
      // Distribute extra space proportionally, with Packing Checklist getting priority
  ...
  const enforceHorizontalConstraints = () => {
    ...
    // Phase 4: Horizontal scroll limitation (max 2 min-width columns visible)
    if (!canFitMinimumWidths(visibleColumns)) {
      const priority = getShrinkingPriority();
      
      // Hide columns by priority until we can fit minimum widths
      ...
      for (const columnToHide of priority) {
        if (columnsToCheck.includes(columnToHide) && columnsToCheck.length > 1) {
          ...
          if (canFitMinimumWidths(columnsToCheck)) {
            // Update visibility to hide the column
            setColumnVisibility(prev => ({
              ...prev,
              [columnToHide]: false
            }));
