...
function App() {
  ...
  if (!tripForm) throw new Error('TripForm context not available');
  const { state } = tripForm;
  const navigate = useNavigate();
  // Only show checklist if user has advanced past the form (step > 1)
  const showChecklist = state.step && state.step > 1;

  // Redirect to /MainLayout when step > 1
  useEffect(() => {
    if (showChecklist) {
      navigate('/MainLayout', { replace: true });
    }
  }, [showChecklist, navigate]);
